...
ediv=null;}
var RvyHideInterval=setInterval(RvyHideElements,50);var RvySubmissionUI=function(){if($('div.edit-post-post-schedule').length){var refSelector='div.edit-post-post-schedule';}else{var refSelector='div.edit-post-post-visibility';if(!$(refSelector).length){refSelector='div.edit-post-post-status h2';}}
if(rvyObjEdit.ajaxurl&&!$('div.edit-post-revision-status').length&&$(refSelector).length){$(refSelector).before('<div class="components-panel__row rvy-creation-ui edit-post-revision-status">'
...
var newPreviewItem='';if(rvyObjEdit.viewTitleExtra){newPreviewItem='<div class="components-menu-group"><div role="group"><div class="edit-post-header-preview__grouping-external">'
...
+rvyObjEdit.viewTitle+'</a></div></div></div>';if(rvyObjEdit.viewTitleExtra&&!$('div.rvy-revision-preview').length){if(rvyObjEdit.viewTitleExtra){$('div.block-editor-post-preview__dropdown').after(newPreviewItem).addClass('rvy-revision-preview');}}}
if(($('div.edit-post-header__settings a.editor-post-preview:visible').length||$('div.block-editor-post-preview__dropdown button.block-editor-post-preview__button-toggle:visible').length)&&!$('a.rvy-post-preview').length){if(rvyObjEdit.viewURL&&$('.block-editor-post-preview__button-toggle').length){if($('div.edit-post-header-preview__grouping-external').length==1){if($('div.edit-post-header-preview__grouping-external a svg').length){$('div.edit-post-header-preview__grouping-external').after(newPreviewItem);}}
if(rvyObjEdit.viewCaption){RvyRecaptionElement('.block-editor-post-preview__button-toggle',rvyObjEdit.viewCaption);$('button.block-editor-post-preview__button-toggle:not(.ppr-purple-button)').removeClass('is-tertiary').addClass('is-secondary').addClass('ppr-purple-button');}}
if(rvyObjEdit.viewTitle){$('div.edit-post-header__settings a.rvy-post-preview').attr('title',rvyObjEdit.viewTitle);}}else{if(!rvyObjEdit.multiPreviewActive){if(!$('a.editor-post-preview').next('a.rvy-post-preview').length){var original=$('div.edit-post-header__settings a.editor-post-preview');$(original).after(original.clone().attr('href',rvyObjEdit.viewURL).attr('target','_blank').removeClass('editor-post-preview').addClass('rvy-post-preview').css('margin','0 10px 0 10px'));if(rvyObjEdit.viewCaption){RvyRecaptionElement('div.edit-post-header__settings a.rvy-post-preview',rvyObjEdit.viewCaption);}

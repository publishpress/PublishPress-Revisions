...
if(rvyObjEdit[rvyObjEdit.currentStatus+'ActionCaption']){var approveButtonHTML='';if(rvyObjEdit.canPublish&&(rvyObjEdit.PendingStatus!=rvyObjEdit.currentStatus)&&('future'!=rvyObjEdit.currentStatus)){approveButtonHTML='&nbsp;<a href="'+rvyObjEdit['pendingActionURL']+'" class="button rvy-direct-approve">'
...
$('a.revision-approve, a.rvy-direct-approve').attr('disabled','disabled');});$(document).on('click','a.save-timestamp, a.cancel-timestamp',function(){wp.autosave.server.triggerSave();});function RvyGetRandomInt(max){return Math.floor(Math.random()*max);}
$(document).on('click','div.postbox-container',function(){$('a.revision-approve, a.rvy-direct-approve').attr('disabled','disabled');});var rvyThumbnail=$('#set-post-thumbnail img').attr('src');setInterval(function(){if($('#set-post-thumbnail img').attr('src')!=rvyThumbnail){$('a.revision-approve, a.rvy-direct-approve').attr('disabled','disabled');}},500);$(document).on('click','a.revision-approve',function(){if($('a.revision-approve').attr('disabled')){return false;}
